...
void CapSense_IntClock_Stop(void) 
{
    ...
    CapSense_IntClock_CLKEN &= (uint8)(~CapSense_IntClock_CLKEN_MASK);
	CapSense_IntClock_CLKSTBY &= (uint8)(~CapSense_IntClock_CLKSTBY_MASK);
}
...
{
    if ((CapSense_IntClock_CLKEN & CapSense_IntClock_CLKEN_MASK) != 0u)
    {
    ...
    if (clkDivider != oldDivider)
    {
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CapSense_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((CapSense_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
{
    CapSense_IntClock_MOD_SRC |= modeBitMask & (uint8)CapSense_IntClock_MODE_MASK;
}
...
{
    CapSense_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(CapSense_IntClock_MODE_MASK));
}
...
{
    return CapSense_IntClock_MOD_SRC & (uint8)(CapSense_IntClock_MODE_MASK);
}
...
void CapSense_IntClock_SetSourceRegister(uint8 clkSource) 
{
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        CapSense_IntClock_MOD_SRC =
            (CapSense_IntClock_MOD_SRC & (uint8)(~CapSense_IntClock_SRC_SEL_MSK)) | clkSource;
    }
